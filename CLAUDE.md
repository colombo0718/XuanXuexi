...
timeout 120 gemini -p "..."                                # 加 timeout 保護，避免等太久（讀大量檔案建議 120s）
```
...
- 翻譯、改寫、潤稿
- **讀取長文件**：單檔超長、或需要一次讀多份文件並整理摘要時，交給 Gemini
...
每次要建立或大幅更新 `PROJECT.md` 前，必須先讓 Gemini 把整個專案讀一遍，再根據他的匯報來撰寫：

```bash
timeout 120 gemini --yolo -p "請完整讀取 C:/Users/USER/xuanxuexi 資料夾下所有文字檔案（跳過圖片與 PDF），然後條列出：1) 專案定位與核心架構 2) 各資料夾與關鍵檔案的用途 3) 目前開發進度 4) 重要技術細節與設計決策 5) 已知問題或待辦事項。請盡量詳細。"
```

Gemini 匯報完後由 Claude 統整，寫入 `PROJECT.md`。
